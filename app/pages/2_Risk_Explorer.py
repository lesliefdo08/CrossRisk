...
try:
    ...
    try:
        ...
        if results_df.empty:
            st.warning("No segments match the selected filters. Try adjusting your criteria.")
            st.stop()
        ...
        # Visualization tabs
        tab1, tab2, tab3, tab4 = st.tabs(["Risk Distribution", "Regional View", "Age Analysis", "Occupation View"])
        ...
        st.error(f"Error loading filtered results: {str(e)}")
        st.warning("Unable to load risk explorer data. Please check your filters or connection.")
