...
st.title("Organization Comparison")
...
try:
    ...
    st.error(f"Error loading overview comparison: {str(e)}")
...
try:
    ...
    st.error(f"Error loading regional comparison: {str(e)}")
...
try:
    ...
    if age_df.empty:
        st.warning("No age group comparison data available.")
    else:
    ...
    st.error(f"Error loading age group comparison: {str(e)}")
    ...
    if correlation_df.empty:
        ...
        with col1:
            ...
            if correlation > 0.7:
                ...
                st.info("Moderate correlation - some alignment in risk assessment")
            else:
                st.warning("Weak correlation - organizations assess risk differently")
...
try:
    ...
    if composite_df.empty:
        st.warning("No composite risk data available.")
    else:
        # Stacked bar chart
        fig = px.bar(
            composite_df,
    ...
    st.error(f"Error loading composite risk insights: {str(e)}")

# Export options
st.markdown("---")
try:
    if 'display_regional' in locals() and not display_regional.empty:
        col1, col2 = st.columns(2)
        ...
        with col2:
            if 'display_age' in locals() and not display_age.empty:
                st.download_button(
